...
import threading
from new_file import AiderRunner
import time
import signal
import queue
import uuid
...
def run_script_w(script_path):
    script_running.set()
    try:
        process = subprocess.Popen(['python', script_path],  # Added -u flag for unbuffered output
...
def handleNewFileScript():
    ...
    runner = AiderRunner(user_prompt)
    result = runner.run()
    
    logger.info("new_file.py execution successful")
    db.add_script_run('new_file.py', result)
...
def run_script(script_name):
    ...
    try:
        ...
        if script_name == 'new_file.py':
            return handleNewFileScript()
        else:
            process, input_queue, output_queue = run_script_w(script_path)
...
def script_input(script_name):
    ...
    try:
        ...
        # First, wait a short time for any immediate output
        time.sleep(0.1)  # Give the script time to process
        ...
        while True:
            try:
                line = output_queue.get_nowait()  # Don't wait, just check what's available
                output.append(line)
                if '?' in line:  # Check for input prompt
                    seen_input_prompt = True
            except queue.Empty:
                break
        
        # If we got output and no new input prompt, return the output
        if output and not seen_input_prompt:
            return jsonify({'response': ''.join(output)})
        
        # If we see a new input prompt, return that
        if seen_input_prompt:
            return jsonify({
                'input_required': True,
                'prompt': ''.join(output),
        ...
        # If we got no output, wait a bit longer for potential input prompt
        start_time = time.time()
        while time.time() - start_time < 1.0:  # Wait up to 1 second for input prompt
            try:
                line = output_queue.get(timeout=0.1)
                output.append(line)
                if '?' in line:  # Found input prompt
                    return jsonify({
                        'input_required': True,
                        'prompt': ''.join(output),
                        'response': ''.join(output)
        ...
        # If we still have output but no input prompt, return it
        if output:
            return jsonify({'response': ''.join(output)})
            
        # If we got nothing at all, the script might be processing
        return jsonify({'response': 'Processing...'})
        
    except Exception as e:
        logger.error(f"Error handling script input: {str(e)}")
        return jsonify({'error': f"Error handling script input: {str(e)}"}), 500
...
def custom_reloader(app):
    ...
    def watch_files():
        ...
        while not stop_event.is_set():
            ...
            if new_files:
                # Check if the new files are Python files created by new_file.py
